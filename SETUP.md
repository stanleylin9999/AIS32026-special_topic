...
從一台乾淨的 Linux 主機（Ubuntu/Debian 系）建出 GRFICSv3 + Mythic 這兩個獨立的 Docker Compose stack。細節架構、port 對照表、攻擊路徑現況看 `README.md`，這份文件只講怎麼把環境生出來。

## 前置需求

- Docker Engine + Docker Compose plugin。Mythic repo 內附 `install_docker_ubuntu.sh`（也有 debian/kali 版本），用 root 跑就會裝好。
- 把自己帳號加進 `docker` group，之後跑 `mythic-cli`、`docker compose` 都不用 sudo：
  ```bash
  sudo usermod -aG docker $USER
  ```
  加完要登出重登（或 `newgrp docker`）才會生效。
- `git` 和 `git-lfs`（GRFICSv3 的模擬畫面用 Unity WebGL assets，靠 git-lfs 抓）。
- Go 編譯環境（`mythic-cli` 是從原始碼編出來的二進位檔，`make` 裡面會呼叫 Go）。

## 抓 repo

GRFICSv3 和 Mythic 是各自獨立的 fork，各自有自己的 git repo，不是 submodule，平行放在同一層目錄下：
...
三個都能開得起來就算建置完成。已知的雷區（`.env` 改密碼後認證失敗、UFW 擋掉 bridge 到 host 的封包）記在 `README.md` 的「Operational notes」，遇到問題先查那邊。
