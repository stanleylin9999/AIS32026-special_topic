...
- **序列曾編排** coil 路徑：FC01 讀 `run_bit` 確認為 TRUE -> FC05 置位 `manual_mode` -> FC16 一次寫入 HR10-13 -> FC01/FC03 讀回驗證 -> 失敗則將 `manual_mode` 寫回 0 回滾，holding register 路徑：FC03 讀 HR1026 存下原值 -> FC06 寫入新 setpoint -> FC03 讀回驗證 -> 失敗則將原值寫回，兩者都沿用樣本 `Task.taskWorker` 已有的 read/dispatch/回報結構，不是外加的框架。
